...
try
sqlite3.execute('update comparated_image set SURF = ? where img_base_id = ? and img_corr_id = ?',S, imgbase.sql_result.id,imgcorr.sql_result.id);
catch
	disp('error during update database')
end
